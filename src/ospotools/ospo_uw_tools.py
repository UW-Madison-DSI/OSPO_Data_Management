...
def uw_validate_owners(conn, owner, relation = 'UW Person', add_repos = True, auth = None):
    ...
    if uwrelationid is None:
        print(f'The relationship provided -- {relation} -- could not be found in the OSPO database.')
        return False
    owner_id = check_owner(conn, owner)
    if owner_id is None:
        print(f'{owner} is not in the OSPO database.')
        return False
    if auth is None:
    ...
    if add_repos:
        ...
        for i in uw_repos:
            repo = insert_repository_db(conn, i.html_url, crawl = False)     
    get_users_repos = """
        SELECT repositoryid
    ...
    print(f'Updating {len(owner_repos)} repositories for {owner}')
